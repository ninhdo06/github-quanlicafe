...
class SignupWindow(QtWidgets.QMainWindow):
    ...
    def signup(self):
        ...
        confirm = self.cf_password.text().strip()

        if not username or not password or not confirm:
            self.error.setText("Vui lòng nhập đầy đủ!")
            return
        if password != confirm:
            self.error.setText("Mật khẩu xác nhận không khớp!")
            return
        ...
        cur = conn.cursor()
        try:
            cur.execute("INSERT INTO login_info (username, password) VALUES (?, ?)", (username, password))
            conn.commit()
            self.error.setText("Đăng ký thành công!")
            # Chuyển về login sau 1 giây
            QtCore.QTimer.singleShot(1000, lambda: self.widget.setCurrentIndex(0))
        except:
            self.error.setText("Tài khoản đã tồn tại!")
        finally:
            conn.close()
